...
const SignOutButton: React.FC = () => {
  ...
  const handleSignOut = () => {
    ...
    navigate(RouterLocationsEnum.authorization);
  };

  return (
    <button
      onClick={handleSignOut}
      className="absolute right-0 top-0 pr-8 pt-8 md:top-2 lg:right-2 xl:right-4 2xl:right-8"
    >
      <svg
        width="18"
        height="18"
      ...
      <span className="hidden rounded-lg border border-white bg-violet p-8 px-4 py-2 text-white hover:bg-violet-800 md:block">
        Выход
      </span>
    </button>
  );
